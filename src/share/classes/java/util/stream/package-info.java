 ...
 *
 * ストリームは<a href="package-summary.html#StreamOps">処理</a>の<em>パイプライン</em>を作成するのに使われる。完全なストリームパイプラインは複数の構成要素からなる。すなわちデータ源(source, {@code Collection}・配列・ジェネレータ関数・IOチャネルなど)・{@code Stream.filter}や{@code Stream.map}といった零個以上の<em>中間処理</em>(intermediate operation)・{@code Stream.forEach}や{@code Stream.reduce}といった終端処理(terminal operation)である。ストリーム処理は引数として<em>関数値</em>(多くの場合ラムダ式であるが、メソッド参照やオブジェクトの場合もある)を取る場合がある。例えば{@code Stream#filter}メソッドに渡す{@code Predicate}のように、関数値は処理の動作をカスタマイズする。
 *
 * <p>中間処理は新しい{@code Stream}を返す。それらは遅延的(lazy)である。{@link java.util.stream.Stream#filter Stream.filter}のような中間処理を実行しても実際のフィルタリングは実行されず、代わりに最初の{@code Stream}の要素のうち与えられた{@code Predicate}にマッチする要素を(走査した際に)含む新しい{@code Stream}を作成する。末端処理が実行されるまでストリームのデータ源の要素は消費しない。
 * 
 * <p>末端処理は{@code Stream}を消費し、成果物を生成するか、副作用を発生させる。末端処理を実行した後はそのストリームはもはや利用できず、元のデータ源から別の処理をやり直すか新しいデータ源を選び別の処理を開始する必要がある。例えば全ての赤いブロックの重さの和を求めた後に全ての青いブロックの重さも求めたい場合は、2つの異なるストリームに対して絞り込み・写像・簡約をする必要がある。
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>この例における逐次版と並列版の唯一の違いは最初の{@code Stream}の生成部分である。{@code Stream}が逐次的に実行されるか並列的に実行されるかは{@code Stream#isParallel}によって決定できる。末端処理が開始された際に、ストリームは逐次的か並列的に実行される。それはストリームの逐次・並列処理に関連する最後の処理によって決まる(ストリームのデータ源か{@code sequential()}メソッドまたは{@code parallel()}メソッドである)。
 *
 ...
 *
 * {@code java.util.stream}パッケージを使うと、{@code ArrayList}などのスレッド安全でないコレクションを含む様々なデータ源に対して、並列な場合も含めてまとめて要素を処理できるようになる。これはストリームパイプラインを実行している間、データ源に対する<em>干渉</em>(interference)を防げる場合のみ可能となる(パイプラインの実行は末端処理が呼び出された際に開始し、末端処理が完了した際に終了する)。
 * 
 ...
 *
 * <p>もちろん、このような処理は簡単な逐次的ループとして次のように何なく実装できる。
 * 
 * <pre>{@code
 *    int sum = 0;
 *    for (int x : numbers) {
 ...
 * }</pre>
 * 
 * しかし、{@link java.util.stream.Stream#reduce 簡約処理}を選択すると、上記のような可変的な累積と比較して重大な利点を得られる。つまり、適切に構築された簡約処理は{@link java.util.function.BinaryOperator 簡約演算子}(reduction operator)が適切な性質を持つかぎり本質的に並列化できる。特に、そのような演算子は<a href="#Associativity">結合的</a>(associative)である必要がある。例えば、数のストリームが与えられた場合に合計を求めたい場合、次ように書ける。
 *
 * <pre>{@code
 ...
 *
 * <p>{@code reduce}の実装は、ストリームの部分部分に対して並列的に処理を実行してその後中間処理をまとめて最終的な正しい答えを得られるため、簡約は上手く並列化できる。上記の元のfor-eachループの代わりに、並列化可能な{@link java.util.stream.Stream#forEach(Consumer) forEach()}メソッドを使ったとしても{@code sum}という累積変数に対してスレッド安全な更新をする必要があり、必要となる同期処理は並列化から得られる性能向上を打ち消してしまうだろう。代わりに{@code reduce}メソッドを使えば簡約処理を並列化する際の一切の面倒を除去でき、ライブラリは余分な同期処理を必要とせずに効率的な並列実装を提供できるようになる。
 *
 ...
 * }</pre>
 *
 * <p>その最も一般的な形式では、{@code <T>}型の要素に対して{@code <U>}型の結果を産出する{@code reduce}処理は3つのパラメータを必要とする。
 * <pre>{@code
 ...
 *
 * <p>より形式的には{@code identity}は統合関数の<em>単位元</em>である必要がある。つまり任意の{@code u}に対して、{@code combiner.apply(identity, u)}は{@code u}であるということである。加えて、{@code combiner}関数は<a href="#Associativity">結合的</a>であり、{@code accumulator}関数と適合する必要がある。任意の{@code u}と{@code t}について次の等式が成り立つ必要がある。
 * <pre>{@code
 ...
 *
 * <em>可変的</em>(mutable)簡約処理は値のストリームを1つの値にするという点では類似しているが、独立した1つの値を生成するのではなく、ストリームの値を処理する中で{@code Collection}や{@code StringBuilder}などの汎用的な<em>結果コンテナ</em>(result container)を変更していく。
 *
 ...
 * }</pre>
 *
 * これでも望む結果は得られるし、並列的にも動かせる。しかし性能は満足いくものではないだろう。このような実装は大量の文字列コピーを実行し、実行時間は要素数に対して<em>O(n^2)</em>となるだろう。より性能面で良い方法は、結果を{@link java.lang.StringBuilder}という文字列を累積していく可変的コンテナに累積していく方法だろう。可変的簡約を並列化する方法として通常の簡約の際と同じ手法が使える。
 *
 ...
 * }</pre>
 * {@code reduce()}と同様に{@code collect}をこのような抽象的な形で表す利点は、並列化を直接適用できる点である。つまり中間結果を並列的に累積し、その後統合する。例えば、ストリーム中の要素の文字列形式を{@code ArrayList}に集めたい場合、次のような明確な逐次的for-each形式で書くこともできる。
 * <pre>{@code
 ...
 *     }
 * }</pre>
 * もしくは、並列化可能な収集形式(collect form)で次のように書ける。
 * <pre>{@code
 ...
 *
 * <p>並行簡約をサポートする{@link java.util.stream.Collector}は{@link java.util.stream.Collector.Characteristics#CONCURRENT}特性の印が付けられている。並行コレクタが与えられているというのは並行簡約のための必要条件であるがそれだけでは十分でない。複数の累積関数が共有コンテナに値を累積している様子を想像してみると、結果が累積される順番は非決定的であるとわかる。それゆえ、並行簡約はストリームを処理する順番が重要でないときにのみ可能となる。{@link java.util.stream.Stream#collect(Collector)}は次の条件が満たされるときにのみ並行簡約を実施する。
 * <ul>
 * <li>ストリームが並列的である。</li>
 ...
 *
 * <p>データ源は直接スプリッテレータを用意する場合がある。その場合、スプリッテレータは走査され・分割し・大きさの見積りを問い合わせられる。これらは末端処理が開始した後に実行されるが、決してその前にはなされない。スプリッテレータは{@code IMMUTABLE}特性か{@code CONCURRENT}特性を公表するよう強く推奨される。もしくは<em>遅延束縛</em>(late-binding)をし、走査され・分割し・大きさの見積りを問い合わせられるまで扱う要素を束縛するべきでない。
 *
