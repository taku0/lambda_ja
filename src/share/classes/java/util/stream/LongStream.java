...
/**
 * プリミティブのlongである要素の列であり逐次的および並列的バルク処理を備える。
...
public interface LongStream extends BaseStream<Long, LongStream> {
     ...
     * }</pre>
     *
     * @param mapper 各要素に適用する、<a href="package-summary.html#NonInterference">非干渉的で状態を持たず</a>新しい値の{@code LongStream}を生成する関数
     * @return 新しいストリーム
    ...
    /**
     * 与えられた<a href="package-summary.html#Associativity">結合的</a>な累積関数を使って、このストリームの要素に<a href="package-summary.html#Reduction">簡約</a>処理を実行して簡約された値があればそれを表す{@code OptionalLong}を返す。これは次と等しい。
     * <pre>{@code
     ...
     * }</pre>
     *
     * <p>{@link #reduce(long, LongBinaryOperator)}のように、{@code collect}処理は追加の同期処理を必要とせずに並列化できる。
     *
     * <p>これは<a href="package-summary.html#StreamOps">末端処理</a>である。
     *
     * @param <R> 結果の型
    ...
    /**
     * このストリームの最小要素を表す{@code OptionalLong}を返すか、もしこのストリームが空であれば空の{@code OptionalLong}を返す。これは<a href="package-summary.html#MutableReduction">簡約</a>の特別な場合であり、次のコードに等しい。
     * <pre>{@code
     ...
     * <p>これは<a href="package-summary.html#StreamOps">末端処理</a>である。
     *
     * @return このストリームの最小要素を表す{@code OptionalLong}、またはこのストリームが空ならば空の{@code OptionalLong}
     */
    OptionalLong min();

    /**
     * このストリームの最大要素を表す{@code OptionalLong}を返すか、もしこのストリームが空であれば空の{@code OptionalLong}を返す。これは<a href="package-summary.html#MutableReduction">簡約</a>の特別な場合であり、次のコードに等しい。
     * <pre>{@code
     ...
     * <p>これは<a href="package-summary.html#StreamOps">末端処理</a>である。
     *
     * @return このストリームの最大要素を表す{@code OptionalLong}、またはこのストリームが空ならば空の{@code OptionalLong}
     */
    OptionalLong max();
    ...
    /**
     * このストリームの要素の各種概要情報を表す{@code LongSummaryStatistics}を返す。これは<a href="package-summary.html#MutableReduction">簡約</a>の特別な場合である。
     *
     * @return このストリームの要素の各種概要情報を表す{@code LongSummaryStatistics}
     */
    ...
    /**
     * このストリームの(出現順順序で)最初の要素を表す{@link OptionalLong}、もしくはストリームが空であれば空の{@code OptionalLong}を返す。このストリームが出現順順序を持たなければ任意の要素が返される場合がある。
     *
     * <p>これは<a href="package-summary.html#StreamOps">短絡的な末端処理</a>である。
     *
     * @return このストリームの最初の要素を表す{@code OptionalLong}、またはこのストリームが空ならば空の{@code OptionalLong}
     */
    OptionalLong findFirst();

    /**
     * このストリームの任意の要素を表す{@link OptionalLong}、もしくはストリームが空であれば空の{@code OptionalLong}を返す。
     *
     * <p>これは<a href="package-summary.html#StreamOps">短絡的な末端処理</a>である。
     *
     ...
     *
     * @return このストリームのある要素を表す{@code OptionalLong}、またはこのストリームが空ならば空の{@code OptionalLong}
     * @see #findFirst()
     */
     ...
     *
     * @return このストリームの要素を{@code Long}にボックス化した要素からなる{@code Stream}
     */
    Stream<Long> boxed();
    ...
    /**
     * 1つの要素を含む逐次的な{@code LongStream}を返す。
     *
     * @param t 1つの要素
    ...
    /**
     * 初期要素{@code seed}に対して関数{@code f}の繰り返しの適用によって生成された無限{@code LongStream}を返す。{@code seed}, {@code f(seed)}, {@code f(f(seed))}などからなるストームを生成する。
     *
     * <p>{@code LongStream}の最初の要素(位置{@code 0})は{@code seed}によって与えられる。{@code n > 0}に対しては、その位置の要素は{@code f}を位置{@code n - 1}の要素に適用した結果である。
     *
     * @param seed 初期要素
     * @param f 新しい要素を生成するために以前の値に適用される関数
     * @return 新しい逐次的な{@code LongStream}
     */
    ...
    /**
     * 各要素が{@code LongSupplier}によって与えられる、逐次的な{@code LongStream}を返す。定数のストリームや乱数のストリームなどを生成するのに向いている。
     *
     * @param s 要素の{@code LongSupplier}
     * @return 新しい逐次的な{@code LongStream}
     */
